...
*)
  TAsteriouDescriptor = class(TFileDescriptor)
  protected
...
(*----------------------------------------------------------------------------*)
 TAsteriouReader = class(TPurchaseReader)
 protected
...
{ TAsteriouDescriptor }
(*----------------------------------------------------------------------------*)
constructor TAsteriouDescriptor.Create;
begin
  inherited;

  FName            := 'Input.Descriptor.ΑΣΤΕΡΙΟΥ';
  FFileName        := 'ΑΣΤΕΡΙΟΥ\*.txt';
  FKind            := fkDelimited;
  FDelimiter       := '#';
...
(*----------------------------------------------------------------------------
To αρχείο της ΑΣΤΕΡΙΟΥ είναι μία γραμμή master και ακολουθούν οι detail.
Η μάστερ ξεκινάει με τους χαρακτήρες  H#
Κάθε detail γραμμή ξεκινάει με D#
Από μπροστά μπορεί να υπάρχουν spaces
----------------------------------------------------------------------------*)
procedure TAsteriouDescriptor.AddFileItems;
begin
  inherited;
...
//  FItemList.Add(TFileItem.Create(itMeasUnit, 2, 9));
  ...
  FItemList.Add(TFileItem.Create(itPrice, 2, 7));
  FItemList.Add(TFileItem.Create(itVAT, 2, 8));  // percent
  FItemList.Add(TFileItem.Create(itDisc, 2, 9));   // disc value
  FItemList.Add(TFileItem.Create(itLineValue, 2, 10));
  FItemList.Add(TFileItem.Create(itMeasUnit, 2, 11));
...
{ TAsteriouReader }
(*----------------------------------------------------------------------------*)
constructor TAsteriouReader.Create(Manager: TInputManager; Title: string);
begin
  inherited Create(Manager, Title);
  FDescriptor := FileDescriptors.Find('Input.Descriptor.ΑΣΤΕΡΙΟΥ');
end;
(*----------------------------------------------------------------------------*)
function TAsteriouReader.GetMaterialCode(SupMatCode, SupCode: string; out MatCode: string; out MatAA: Integer): Boolean;
...
// Αντικατάσταση κωδικών για ΚΟΤΟΛΕΤΑ ΠΑΝΕ LID ΝΩΠΗ 1,1 κιλ.
  if (SupMatCode = '561103') then
    SupMatCode  := '561101';
...
(*----------------------------------------------------------------------------*)
function TAsteriouReader.GetPayType: string;
begin
  Result := 'ΕΠΙ ΠΙΣΤΩΣΗ';
end;
...
function TAsteriouReader.GetLineValue: Double;
...
(*----------------------------------------------------------------------------*)
function TAsteriouReader.GetDocNo: string;
var
  s: string;
begin
...
(*----------------------------------------------------------------------------*)
function TAsteriouReader.DocStrToDate(S: string): TDate;
begin
  // 01/11/16
...
initialization
  FileDescriptors.Add(TAsteriouDescriptor.Create);
