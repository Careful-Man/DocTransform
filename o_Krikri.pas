...
type
(*----------------------------------------------------------------------------*)
  TKriKriDescriptor = class(TFileDescriptor)
  protected
...
(*----------------------------------------------------------------------------*)
 TKriKriReader = class(TPurchaseReader)
 protected
...
{ TKrikriDescriptor }
(*----------------------------------------------------------------------------*)
constructor TKriKriDescriptor.Create;
begin
  inherited;

  FName            := 'Input.Descriptor.KRIKRI';
  FFileName        := 'йяийяи-ц\*.csv';
  FKind            := fkDelimited;
  FDelimiter       := ';';
//  FKind            := fkFixedLength;
  FSchema          := fsSameLine;
...
(*----------------------------------------------------------------------------*)
procedure TKriKriDescriptor.AddFileItems;
begin
  inherited;
...
{ TKrikriReader }
(*----------------------------------------------------------------------------*)
constructor TKriKriReader.Create(Manager: TInputManager; Title: string);
begin
  inherited Create(Manager, Title);
  FDescriptor := FileDescriptors.Find('Input.Descriptor.KRIKRI');
end;
(*----------------------------------------------------------------------------*)
(* оИ ДЩО ПЯЧТОИ ВАЯАЙТчЯЕР АПЭ ТО ПЕДъО 9-1.                                 *)
function TKriKriReader.GetDocType: string;
var
  s: string;
begin
...
(*----------------------------------------------------------------------------*)
(*function TKriKriReader.GetGLN: string;
begin
  Result := TrimLeftZeroes(GetStrDef(fiGLN));
end;*)
(*----------------------------------------------------------------------------*)
function TKriKriReader.GetDocNo: string;
var
  s: string;
begin
  s := GetStrDef(fiDocId);
...
(*----------------------------------------------------------------------------*)
function TKriKriReader.GetPrice: Double;
var
  s: string;
begin
...
(*----------------------------------------------------------------------------*)
function TKriKriReader.GetMaterialCode(SupMatCode, SupCode: string; out MatCode: string; out MatAA: Integer): Boolean;
...
begin
  if LeftString(SupMatCode, 1) = '1' then // аПЭ 1 АЯВъФОУМ ОИ ЙЫДИЙОъ пАЦЫТЧМ ЙАИ ДЕМ ТА ХщКЫ.
  begin
...
//                     [SupCode, Utls.DateToStrSQL(DocDate, False), RelDoc, SupMatCode]))
  ...
  begin
// аМТИЙАТэСТАСГ ЙЫДИЙЧМ ЦИА паид.епид.PJ MASKS лпам пяосх 3в150ця йяи-йяи (-0,60E)100% екк
    if (SupMatCode = '2040615') then
      SupMatCode  := '2040776';
...
(*----------------------------------------------------------------------------*)
function TKriKriReader.DocStrToDate(S: string): TDate;
var
  Y, M, D: string;
begin
  // 21/07/2012
...
initialization
  FileDescriptors.Add(TKriKriDescriptor.Create);
